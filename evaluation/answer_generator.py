...
class AnswerGenerator:
    def __init__(self, model, tokenizer, device, dataset, instruction, context_lengths, max_context_length, max_new_tokens, chat_model, sys_prompt):
        self.model = model
    ...
    def create_prompt_with_chat_template(self, sample):
        ...
        inputs = self.tokenizer.apply_chat_template(messages, tokenize=True, add_generation_prompt=True, 
                                                    truncation=True, max_length=self.max_context_length, return_tensors="pt", return_dict=True).to(self.device)
        return inputs
